...
> **Note:** `pharmaverseadam::adsl` does not contain `BMIBL`. We use `AGE` as the only continuous variable available.
...
```{r ard-continuous}
# ard_continuous() generates a tidy ARD for numeric variables
...
```{r ard-ttest}
# cardx extends cards with inferential statistics
...
## Step 8: Combine All ARDs

```{r ard-combine}
# bind_ard() stacks multiple ARD objects into one
ard_all <- cards::bind_ard(
  ard_cont,
  ard_cat,
  ard_miss,
  ard_t,
...
cat("Total ARD rows:", nrow(ard_all), "\n")
cat("Contexts present:\n")
ard_all |>
  dplyr::count(context)
```
...
## Step 9: Inspect the Full ARD

```{r ard-inspect}
# Key columns of the combined ARD
ard_all |>
  dplyr::select(group1, variable, context, stat_name, stat_label, stat) |>
  head(20)
```
...
```{r ard-extract}
# Pull mean AGE per treatment arm from the ARD
ard_all |>
  dplyr::filter(
    variable   == "AGE",
    stat_name  == "mean",
...
```{r ard-format}
# cards::apply_fmt_fn() applies formatting functions stored in fmt_fn column
ard_formatted <- ard_all |>
  dplyr::filter(
    variable  == "AGE",
    context   == "continuous",
...
```{r ard-summary-table}
# Use the ARD to build a readable summary manually
mean_age <- ard_all |>
  dplyr::filter(variable == "AGE", stat_name == "mean", context == "continuous") |>
  dplyr::mutate(
    treatment = as.character(group1_level),
    mean_age  = round(unlist(stat), 1)
  ) |>
...
```{r ard-regression}
# Logistic regression: predict SEX from AGE
# (simple example to demonstrate ard_regression)
...
# Check 1: No error records in combined ARD
check1 <- ard_all |>
  dplyr::filter(!is.null(error) & !sapply(error, is.null))
cat("Check 1 - ARD error records:", nrow(check1), "\n")
...
# Check 3: Categorical stats sum to N
cat_n <- ard_all |>
...
cat("ARD contexts built:\n")
ard_all |>
  dplyr::count(context) |>
  print()

cat("\nTotal ARD rows:", nrow(ard_all), "\n")
cat("Variables covered: AGE, SEX, RACE, AGEGR1, ETHNIC\n")
cat("Tests: t-test, chi-square, Wilcoxon rank-sum\n")
cat("Regression: logistic (age + treatment)\n")
```
...
```{r export}
# ARD is a tibble - flatten list columns for CSV export
ard_export <- ard_all |>
  dplyr::mutate(
...
cat("✓ ard_day24.csv exported\n")
cat("Rows:", nrow(ard_export), "\n")
```
...
3.  `bind_ard()` stacks multiple ARD objects into one combined dataset
4.  ARD `stat` column is a **list column** — use `unlist()` or `apply_fmt_fn()` to extract
5.  `pharmaverseadam::adsl` has no `BMIBL` — use only confirmed columns
6.  The ARD is the **single source of truth** for all downstream tables and figures
